...
So, each row of the matrix A is projected on the vector vi which is like finding the component of matrix A along the vector $v_i$. Consider one row, say $$a_j$$, of A. Since $$v_1,v_2....v_r$$ span the space of all rows of A, and as already mentioned umpteen times above these vectors are orthogonal so whatever was projected on $v_1$ will be unrelated to what will be projected to other vectors. Thus, we can say for each row $$a_j$$, $$\sum_{i=1}^{r}(a_j.v_i )^2 =|a_j |^2$$
